# Ejabberd-Practice-in-iOS-Application
=====================================
...
- **Fault-tolerant**  
  ...
  without disruption. In addition, nodes also can be added or replaced âon
  the flyâ.
...
==============
  ...
  ejabberd community installer comes with Preference Pane to start/stop ejabberd.

  Preference Pane is installed in $HOME/Library/PreferencePanes/ejabberd.prefPane

  If you run Mac OS X >= 10.5.0 and need Preference Pane, you must open the
  Terminal application and type the following commands:

    $ sudo -s
    <type admin password>
    $ cd /Library/PreferencePanes
    $ ln -s $HOME/Library/PreferencePanes/ejabberd.prefPane
